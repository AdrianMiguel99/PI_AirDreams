...
import ReservationDetailsPage from "../ReservationDetailsPage.vue";
...
describe("ReservationDetailsPage.vue - unit tests de métodos", () => {
  ...
  it("mapea correctamente una reserva directa", () => {
    ...
    expect(result.tickets[0].passengerName).toBe("Juan Pérez");
  ...
  it("mapea correctamente una reserva con escala", () => {
    ...
    expect(result.tickets[0].id).toBe(1);
    expect(result.tickets[0].passengerName).toBe("Ana Obando");
    expect(result.tickets[1].id).toBe(2);
    expect(result.tickets[1].passengerName).toBe("Carlos Mora");
  });
  ...
  it("carga los detalles de la reserva desde el backend", async () => {
    ...
    expect(wrapper.vm.reservation.tickets[0].passengerName).toBe("Ana Obando");
  });
